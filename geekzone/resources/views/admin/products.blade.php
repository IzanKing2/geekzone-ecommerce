...
<div class="layout-sidebar">
    ...
    <main class="main-content">
        ...
        <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem">
            <div>
                <p class="page-eyebrow" style="padding-top:0">Gestión de inventario</p>
                <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2.5rem;letter-spacing:2px">Productos <span style="color:var(--cobalt-light)">CRUD</span></h2>
            </div>
...
<script>
    ...
    async function createProduct() {
        ...
        if (!name)  { document.getElementById('add-name-error').textContent  = 'El nombre es obligatorio.';  document.getElementById('add-name-error').style.display  = 'block'; valid = false; }
        if (!catId) { document.getElementById('add-cat-error').textContent   = 'Selecciona una categoría.';  document.getElementById('add-cat-error').style.display   = 'block'; valid = false; }
        if (!price) { document.getElementById('add-price-error').textContent = 'El precio es obligatorio.';  document.getElementById('add-price-error').style.display = 'block'; valid = false; }
        if (!valid) return;
        ...
        const res = await fetch(API_PROD, {
            ...
            body: JSON.stringify({
                name, category_id: parseInt(catId), price: parseFloat(price),
                stock: parseInt(stock) || 0, featured,
                image_url: image || null, description: desc || null,
            }),
        });
    ...
    async function deleteProduct(id, name) {
        ...
        const json = await res.json();
        if (!res.ok) { showAlert(json.message || 'No se pudo eliminar.', 'error'); return; }
